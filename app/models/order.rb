class Order < ActiveRecord::Base
  ...
  has_many :tshirts, :through => :order_items

  validates_presence_of :order_items,
                        :message => 'Your shopping cart is empty! ' +
                                    'Please add at least one t-shirt to it before submitting the order.'
  ...
  def active_merchant_payment
    ...
    # order information
    details = {
      :description      => 'Futboleros t-shirts',
      :order_id         => self.id,
      :email            => email,
      :ip               => customer_ip,
